...
import { getGoogleCallbackURL, getGoogleRedirectUrl } from "../googleHelper.mjs";
...
const rest: RestDefinition = {
  ...
  docs: {
    tags: ["Google Login"],
    description: "Callback login (Web)",
    responses: {
  ...
  middlewares: [
    (req, res, next) => {
      ...
      passport.authenticate("google", {
        callbackURL,
        failureRedirect: "/api/google/login/failed/web",
        session: false,
  ...
  entryPoint: async (req, res) => {
    ...
    const webRedirectUrl = getGoogleRedirectUrl("/web");
    ...
    try {
      ...
      const redirectUrl = `${webRedirectUrl}?access_token=${token.accessToken}&refresh_token=${token.refreshToken}`;
      res
      ...
      console.log("[google/callback/web] error:", error);
      res.status(301).redirect(`${webRedirectUrl}?login_error=INVALID_CREDENTIALS`);
    }
  },
};
export default rest;
