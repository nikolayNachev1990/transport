...
const env = loadConfig(
  {
    ...
    AUTH_KAFKA_GROUP: { required: true, description: "Kafka consumer group name" },
    AUTH_KAFKA_GROUP_ID: { required: false, description: "Kafka consumer groupId override" },
  },
  process.env,
);
...
// callback, keyed by its own filename (which is the topic name, e.g.
// qrcodes.clear.queue.mts -> "qrcodes.clear.queue"). createBroker only
// actually registers a consumer for a topic that's both present here AND
// listed against this service's group in Events/<topic>.mjs's
// `consumers` map — an entry here for a topic Events/ doesn't route to
// this group is simply unused, not an error.
...
export default async function brokerConfig(): Promise<BrokerConfig> {
  return {
    group: env.AUTH_KAFKA_GROUP,
    groupId: env.AUTH_KAFKA_GROUP_ID,
