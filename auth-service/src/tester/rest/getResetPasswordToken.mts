...
const rest: RestDefinition = {
  route: "/user/reset/password/token/:userId",
  method: "GET",
  ...
  docs: {
    tags: ["Tester"],
    description: "Get User Reset Password Token (Only For Tester)",
    responses: {
  ...
  entryPoint: async (req, res) => {
    ...
    const getToken = (await db.findByWhere<{ code: string }>("password_resets", {
      user_id: userId,
      completed_at: null,
    })) as { code: string } | null;
