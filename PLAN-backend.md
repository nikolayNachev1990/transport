...
| База | **отделна база на услуга, отделен Postgres контейнер на услуга** (не един споделен инстанс) — изолация при повреда: срив, ъпгрейд или възстановяване на една база засяга само нейната услуга, не цялата система |
...
| Фронтенд хостинг | `web` контейнер в същата Docker мрежа (SSR чете Hasura вътрешно); Electron/Expo се изпълняват на устройството на потребителя, извън мрежата — виж „Фронтенд — три нива“ |
| Сървър | Hetzner CX42 (8 vCPU, 16 GB) — **една машина**, всичко в една Docker мрежа |
...
```
   electron (десктоп)      expo (всички роли)     браузър
        │                        │                   │
        └────────────────────────┼───────────────────┘
                          (публичен адрес)
                                  ▼
             ...
             ▼                    ▼                              ▼
        ┌─────────┐      ┌──────────────┐              ┌──────────────────┐
        │  web    │      │    hasura    │              │  пишещи услуги   │
        │ (SSR)   │─────▶│   (READ, +   │              │     (WRITE, през │
        └─────────┘ int. │   Actions)   │              │   Hasura Actions)│
                          └──────┬───────┘              └────────┬─────────┘
                                 │                                │
                    ┌────────────┴───────────┐                   │
                    ▼                        ▼                   │
   ...
   web контейнерът (SSR) вика Hasura директно по вътрешното име
   (http://hasura:8080), без да излиза през nginx — виж „Фронтенд — три
   нива“ по-долу за какво остава вътрешно и какво минава по публичния адрес.

   Файлове:  клиент ──подписан URL──▶ Cloudflare R2  (не минава през nginx;
   URL-ът се строи от ПУБЛИЧНИЯ адрес, никога от името на контейнер)
...
Тази секция описва само какво бекендът трябва да поддържа заради
фронтенда — самите `web`/`electron`/`expo` кодови бази не са част от този
план (`PLAN-backend.md` е само за бекенда) и имат нужда от собствен план,
когато им дойде редът.

| Ниво | Какво е | Мрежа |
|---|---|---|
| `web` | Контейнер в същата Docker мрежа. SSR частта вика `http://hasura:8080` директно, вътрешно — не излиза през nginx за собствените си server-side заявки. | Вътрешна (+ публична за самите браузър заявки) |
| `electron` | Обвивка около същото web приложение, същата кодова база. | На устройството на потребителя, извън мрежата |
| `expo` | Мобилно приложение за **всички** роли — собственик, ръководител транспорт, диспечър, счетоводител, шофьор — не само за шофьори. | На устройството на потребителя, извън мрежата |

Следствия за бекенда:

- **Expo вижда пълния API** — Hasura за четене и Actions за писане,
  ограничени само от ролята в токена (Hasura permissions, етап 22). Няма
  отделен, „олекотен" API за мобилно — същата повърхност като web/electron.
- **И двата входни пътя от етап 12–13 живеят в едно и също приложение**:
  имейл+парола за офис ролите, шестцифрен `driver_code` за шофьорите.
  Кой екран се показва зависи от ролята в токена, не от устройството —
  бекендът не различава „мобилен" клиент по никакъв специален начин.
- **Electron и Expo са извън мрежата** — за тях всичко минава през nginx
  по публичния адрес. `tms-core/http` (REST през Hasura Actions) и самите
  Actions трябва да работят еднакво добре по вътрешно име
  (`http://<service>:PORT`, когато ги вика Hasura или друга услуга) и по
  публичния адрес (когато ги вика Electron/Expo директно — ако изобщо е
  позволено; по подразбиране всичко минава през Hasura/nginx, виж по-долу).
- **Никой URL към клиент не съдържа име на контейнер.** Подписан линк към
  R2, линк за споделяне, callback адрес — всички се строят от публичния
  адрес, зададен като конфигурационна променлива (`PUBLIC_BASE_URL` или
  еквивалент), никога от `hasura:8080`/`<service>:PORT`. Вътрешните имена
  съществуват само за трафик, който никога не напуска Docker мрежата.

### Услуги и техните бази

Всяка база е в собствен Postgres **контейнер** (не таблица в споделен
инстанс) — изолация при повреда: срив, ъпгрейд или възстановяване на един
контейнер не засяга другите. „Контейнер" по-долу е и docker-compose
service name-а; „База" е името на самата Postgres база вътре в него
(и на ролята, която я притежава — вижте stage 10 за точния механизъм).

| Услуга | Postgres контейнер | База (роля = собственик) | Отговорност |
|---|---|---|---|
| `auth-service` | `auth-db` | `auth_db` | наематели, потребители, роли, сесии, кодове за шофьори |
| `fleet-service` | `fleet-db` | `fleet_db` | камиони, ремаркета, шофьори, документи и срокове |
| `order-service` | `order-db` | `order_db` | клиенти, курсове, спирки, назначения, статуси |
| `billing-service` | `billing-db` | `billing_db` | фактури, плащания, разходи, рентабилност |
| `files-service` | `files-db` | `files_db` | метаданни на файловете, подписани URL-и |
| `track-service` | `track-db` | `track_db` | GPS точки, сегменти движение/престой |
| `routing-service` | `routing-db` | `routing_db` | кеш на геокодиране и маршрути, отклонения |
| `notify-service` | `notify-db` | `notify_db` | шаблони, изпратени известия, предпочитания |
| `ai-service` | `ai-db` | `ai_db` | единствената точка към AI, лог на разхода |
| `doc-service` (Python) | — | — | обработка на изображения, оценка за четимост |
| `query-service` | `query-db` | `query_db` | проекции, справки, ресинхронизация; и вторична, четяща роля за `hasura` (виж по-долу) |
...
| `hasura` | `hasura-metadata-db` | `hasura_metadata_db` | собствен каталог (tracked tables/permissions/actions) — отделно от всички бизнес бази |
...
Redis, само за REST: `restIdempotence` по `Idempotency-Key`.

- Ключът се заема атомарно (Redis `SET NX`, през Lua за целия preview-conflict
  преглед в едно) преди да тръгне работата — никога записван след нея.
- Три състояния: `in_progress`, `completed`, `failed`. Повторение при
  `in_progress` → `409`, веднага, без чакане. `completed` → връща същия
  запазен отговор (тяло + статус), не празно `200`. `failed` (необработена
  грешка/инфраструктурен провал — не бизнес 4xx, който си е валиден,
  детерминиран отговор) позволява повторен опит със същия ключ.
- Пази се и цялото тяло на отговора, не само маркер за успех — иначе
  повторението няма какво да върне.
- Хеш на request body-то се пази до ключа; същия ключ с различно тяло → `422`.
- Обхват на ключа: `tenant_id` + endpoint (route pattern) + `Idempotency-Key`.
- При паднал Redis — конфигурируемо на ниво endpoint: `fail-closed` (откажи
  заявката — за пари) или `fail-open` (пусни я без идемпотентност — за
  некритични неща като качване на файл).

**Проверката на консюмъра по `event_id` не е тук и не е Redis.** Тя е
`processed_events` в `query_db`, в **същата транзакция** като проекцията —
виж етап 19. Redis идемпотентност пази REST заявки; консюмърската
идемпотентност пази проекции, и трябва да е транзакционна с тях, което Redis
структурно не може да гарантира.
...
Един `postgres:16` **контейнер на база** (11 общо — 10 услуги +
`hasura-metadata-db`, вижте „Услуги и техните бази" по-горе), плюс
споделени `redis:7`, `redpanda`, `minio`, `hasura`, `nginx`. Всеки Postgres
контейнер: собствен named volume, healthcheck, `deploy.resources.limits`.
Всичко в една Docker мрежа на една машина (без втора машина за Postgres).
Healthcheck на всяка, `depends_on: service_healthy`.
...
PDF → изображения. Изход: `readable | poor | unusable` + код на причината
(`BLURRY | TOO_DARK | CROPPED | LOW_RESOLUTION | ...`), не изречение — превод
на кода към текст е задача на фронтенда (i18n), не на `doc-service`.
...
### 52. `notify-service`
Имейл (Resend/Postmark), push (Expo), уеб. Шаблоните се идентифицират с
`template_key` (не с текст) и съществуват в 4 езикови варианта; езикът на
изпращане се избира по езика на получателя (за шофьори — `drivers.language`,
етап 23; за останалите роли езикът е поле в notify preferences, уточнява се
на етапа, в който `notify_db` получи собствена схема), не по подателя или по
наемателя.
...
# ФАЗА 10 — search-service (етапи 57–62)

**Не се строи преди да е доказано нужна.** До ~50 наематели търсенето минава
през `pg_trgm` индекси директно в `query_db` (клиенти/курсове/автопарк/
шофьори/фактури вече са там) — `WHERE name % 'заявка'` с trigram GIN индекс
покрива обема напълно. Elasticsearch/`search-service` се пускат едва когато
конкретна `pg_trgm` заявка е измерено бавна в продукция (не предположение,
не „ще стане бавно" — реален `EXPLAIN ANALYZE` над реални данни). Затова тази
фаза не е на критичния път и не се изпълнява, докато не бъде поискана изрично.

Пълнотекстово търсене над Elasticsearch, когато дойде моментът. Услугата
**не е източник на истина**: винаги напълно възстановима от събития и от
`/internal/snapshot/:table` на останалите услуги. Няма собствена Postgres
база.

### 57. Elasticsearch инфраструктура и индекси
`docker-compose` добавя Elasticsearch `9.5.3` (single-node, само вътрешна
мрежа) с healthcheck. Индекси, по един на тип обект (аналогично на
`query_db` — не денормализирани изгледи), всеки документ носи `tenant_id`:
...
| `search_orders` | `order_db.orders` + `order_stops` | `order_no`, `external_ref`, слепен текст от адресите на спирките, `status`, `client_id`, `driver_id`, `vehicle_id`, `agreed_price` | търсене по номер на курс или маршрут |
...
| `search_invoices` | `billing_db.invoices` | `invoice_no`, `client_id`, `status` | търсене на фактура по номер |

**Без денормализирано име на клиент в `search_orders`/`search_invoices`.**
Съзнателно — иначе преименуване на контрагент (или сливане на два дублирани
записа) изисква масов преиндекс на всеки индекс, който го копира, а планът не
описва такъв процес. Търсенето по клиент е **на две стъпки**: първо
`search_clients` (свободен текст, връща `client_id`, разграничен по
`country`/`address` за клоновете), после списъкът с курсове/фактури на този
клиент идва от Hasura с `WHERE client_id = ...` — обикновен filter по
съществуваща колона, не search. `client_id` в `search_orders`/
`search_invoices` служи само за филтриране, никога за текстово съвпадение.
...
Буферът е в собствена Redis инстанция (не споделя с BullMQ/idempotence),
пусната с `appendonly yes` (AOF). Загубата на буфер преди indexing не губи
бизнес данни (всичко е възстановимо от събития/resync), но AOF пести
ненужен ресинк при рестарт на Redis.
...
Метрика (Prometheus, виж и етап 55): `search_buffer_size{index}` — текущ
брой чакащи записи в буфера, отчитан на всеки цикъл на крона. Растящ без
предел буфер = крон-ът е спрял или ES е недостъпен; аларма при продължителен
растеж, не при единично покачване.
...
### 62. Разминаване между индекс и източник
Нощна задача (аналогична на етап 21), за всеки индекс: брой документи в
Elasticsearch срещу брой редове в съответния източник (`order_db.clients`,
`order_db.orders`, `fleet_db.vehicles`, `fleet_db.drivers`,
`billing_db.invoices`, филтрирано по `tenant_id`). Разлика над праг →
известие; повтарящо се разминаване на един и същ индекс → автоматично
пуска resync (етап 61) за него. Без това буферна грешка, изядена мълчаливо,
прави индекса тихо непълен за неопределено време.
...
прекъсване на четенето; нощната проверка открива изкуствено вкарано
разминаване (изтрит документ директно от ES) и го сигнализира.
