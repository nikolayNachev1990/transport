...
async function publishUpserted(row: Record<string, unknown>): Promise<void> {
  await broker.send("fleet.damage_report.upserted", {
    ...
    third_party_involved: row.third_party_involved ?? null,
    police_report_number: row.police_report_number ?? null,
    european_accident_statement: row.european_accident_statement ?? null,
    insurance_document_id: row.insurance_document_id ?? null,
