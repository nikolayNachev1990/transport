...
def _prompt_text(allowed_types: list[dict], hints: dict) -> str:
    types_json = json.dumps(allowed_types, indent=2)
    hints_json = json.dumps(hints or {})
    return (
        "You are looking at a photo or scan of a transport-company document "
        "(vehicle registration, insurance, driving licence, permit, etc.). "
        "Identify which of the following document types it is, then extract "
        "every field you can read.\n\n"
        f"Possible types (each with its own attributes_schema — fill 'fields.attributes' "
        f"using ONLY the property names from the chosen type's attributes_schema, nothing else):\n{types_json}\n\n"
        f"Caller-supplied hints (may be empty; treat as a suggestion, not ground truth):\n{hints_json}\n\n"
        "Use the submit_extraction tool to report your result. If a field isn't visible or "
        "you aren't confident, leave it null rather than guessing. Dates must be ISO 8601 "
        "(YYYY-MM-DD)."
    )
...
def extract(file_bytes: bytes, mime_type: str, allowed_types: list[dict], hints: dict) -> dict:
    """Returns {"tool_input": {...}, "input_tokens": int, "output_tokens": int}."""
    ...
    prompt = _prompt_text(allowed_types, hints)
