...
### 3.1 Опашка и паралелна обработка (`dispatcher.py`, `kafka_client.py`)

Kafka остава трайната опашка; вътре в услугата има работна опашка:

- **Работници** (`DOC_WORKERS`, по подразбиране 2): нишки — OCR е външен процес (tesseract), AI е
  мрежов вход/изход. Всеки работник може да държи разкодирана снимка (до ~80 MB) — при повече
  работници се вдига `DOC_SERVICE_RAM_MAX`. `OMP_THREAD_LIMIT=1`: иначе всеки tesseract вдига нишка
  на ядро и няколко работника се бавят взаимно.
- **Справедливост по фирма:** отделна FIFO опашка за всяка `company_id`, обслужване кръгово, най-много
  `DOC_PER_COMPANY_MAX` (2) едновременно за една фирма. Измерено: при 6 документа на голяма фирма и 1
  на малка, малката получи първия свободен слот след двата активни на голямата, а не след шестте.
- **Обратен натиск:** при пълен буфер (`DOC_MAX_BUFFER`, 40) консумацията се паузира (`pause`), но
  `poll` продължава, за да не се смята услугата за мъртва.
- **Офсети:** записва се само непрекъснатата завършена част на всяка партиция (`OffsetTracker`);
  завършен офсет 7 при работещ 5 не бива да губи 5 при срив. При пренареждане на групата буферираните
  задачи се връщат, изпълняващите се се довършват.
- **Провал:** задача с грешка се повтаря веднъж, след това се отговаря с `doc.extraction.failed`
  (`DOC_INTERNAL_ERROR`) и се брои за завършена — никога не блокира партицията.
- **Спиране (SIGTERM):** довършват се изпълняващите се, записват се офсетите, групата се напуска чисто.
- **Ред на парсерите за снимка:** първо правдоподобните по вече прочетения текст; останалите само ако
  нищо не е намерено (снимка може изобщо да няма четим текст — MRZ, карта на тъмна маса).
...
Независимо от четеца: медианна височина на буквите (tesseract) и рязкост (Лапласиан).
- **Под 0.35 (`UNREADABLE`)** — нищо четимо не може да се възстанови: без Ниво 1, без втора проверка
  (по-силният модел не прави 4-пикселова буква четима).
- **От 0.35 до 0.6** — работата продължава, но всяка увереност (на Ниво 1 и на AI) и `readability_score`
  се ограничават до качеството, така че локално четене никога не достига 0.7, нужните за самостоятелен
  отговор, а човекът вижда полето отбелязано за проверка.
- Стойност, която не минава формата си (VIN не от 17 знака, невалидна дата), получава увереност ≤ 0.2.

Миниатюра 276×393 на протокол от тахограф получава 0.4; AI по нея върна грешен тип и номер със
самоувереност 0.85. Талон част II с MRZ (1118×458) също получава 0.4, но там текстът е четим:
резултатът е `hybrid` с верен VIN, увереност 0.4 и Opus като втора проверка.
...
| DOC_INTERNAL_ERROR | необработена грешка в услугата след повторен опит (бъг, не проблем на файла) |
...
## 11. ЕТАПИ — състояние към 2026-09-19

| # | Етап | Състояние |
|---|---|---|
| 1 | Инфраструктура: Kafka, S3 (GetObject-only), AI клиент, health, compose, AI-only път | **готов**, проверен през реален Kafka/S3/Anthropic |
| 2 | Generic OCR, зареждане на файлове (изображение, PDF, docx, doc, xlsx, HEIC), MRZ | **готов** (MRZ само български талон част II) |
| 3 | Регистрационен талон (BG част I/II) | **частично**: BG формат; други държави → AI (плочи по български шаблон) |
| 4 | Застраховки (ГО, каско, зелена карта, ЧМР застраховка, товарна) | **не е направен**: няма попълнени образци (виж РАВНОСМЕТКА) |
| 4b | Технически преглед | **готов за BG** (роден текст и сканове); други държави → AI |
| 5 | Останали за превозно средство: тахограф протокол, ADR, лиценз на Общността, ЕКМТ, винетка, ATP | **не е направен** (XL сертификат **готов**); всичко останало → AI |
| 6 | Шофьор и фирма: книжка, карта за квалификация, тахографска карта | **готов** (ЕС номерирани полета); ADR за водач, A1, разрешения → AI |
| 7 | Хардинг: `doc.ai_usage.recorded`, опашка, качество, валидация, ескалация | **готов** (виж §3.1, §4.7, §4.8) |
| 8 | sha256/preview/thumbnail/PDF split | извън обхвата |

---

## РАВНОСМЕТКА — какво е направено и какво остава (2026-09-19)

### Направено в тази фаза (всичко комитнато, 93 теста в контейнера + 85 в `tester/`)

- **Предпоставки в другите сервизи:** `upload.completed` носи `company_id` и `path`; fleet-service пълни
  `files` (с `storage_key`) и праща `file_key` + пълните `allowed_types`; Hasura ролите на фирмата вече
  достигат `upload_create/complete`; `document_extractions.engine` допуска `hybrid` (миграция).
- **doc-service:** Ниво 1 (талон BG, книжка/карта за квалификация/тахографска карта, технически преглед BG,
  Code-XL), хибриден режим, оценка на качеството, валидатори, подготовка на вход за AI (HEIC, ограничение
  на размера, до 3 страници PDF), Sonnet по подразбиране с проверима ескалация към Opus, правила за
  полетата по тип, опашка със справедливост по фирма, обратен натиск и безопасни офсети.
- **Бъгове, намерени само с реални файлове през Kafka:** `hybrid` липсваше в Node схемата и в CHECK на
  базата; `auto.offset.reset=latest` губеше заявки при рестарт; OOM при 512 MB на снимка 6543 px; API
  отхвърля изображения над лимита; VIN от OCR с Z/2 излизаше грешен с увереност 0.75.
- **Инструменти:** `tools/eval_ai.py` (модели), `tools/eval_level1.py` (Ниво 1 сам), `tools/scan_corpus.py`,
  тестов корпус в `tester/fixtures/` (само локален).

### Остава — зависи от документи, които публично не съществуват

1. **ГО, каско, зелена карта, товарна/ЧМР застраховка, винетка:** няма публични попълнени образци
   (застрахователите показват само общи условия). Трябват реални от автопарка в
   `tester/fixtures/private/insurance/` с `.expected.json`. До тогава → AI.
2. **Протокол от контролна проверка на тахограф:** единствената налична снимка е 276×393 px (нечетима,
   качество 0.3). Трябва снимка над 1000 px. Типът `tachograph_calibration` няма парсер.
3. **Реални документи от работеща фирма** (обещани): талони, книжки, преглед, ГО — за настройка и за първа
   истинска оценка на точността. Всички измервания досега са върху образци и синтетично повредени копии
   (завъртане, тъмна маса, наклон), не върху истински снимки от телефон.
4. **Твоята книжка** (снимките от чата не са на диска): ако се иска в теста → `private/driving_licences/`.

### Остава — липсващи парсери (в момента минават през AI, ~$0.014/док.)

Лиценз на Общността и заверено копие, ЕКМТ/двустранни разрешителни, ATP, ADR (превозно средство и
водач), Crit'Air/Umweltplakette и градски разрешителни, договори за лизинг/наем, A1, декларация за
командироване. За повечето няма образци с попълнени полета; лицензът на Общността има само празния модел
от Регламент 1072/2009.

### Остава — известни ограничения на самите парсери

- **MRZ Z срещу 2** (OCR-B): tesseract не ги различава (гласове 10:17 при истина Z); покрива се от AI,
  валидатора и ограничението на VIN. Нужен е специализиран OCR-B модел или шаблонно сравнение на знаци.
- **VIN от OCR** с Z/2/S/5/B/8/G/6 никога не отговаря сам (таван 0.6) → почти всеки талон минава през AI.
- **Регистрационен номер** само по български шаблон; чужди талони се разпознават по VIN, номерът иде от AI.
- **Ниво 1 сам отговаря на 3 от 13** файла (книжка BG, книжка IT, карта за квалификация DE); XL сертификатите
  и прегледите се четат точно, но с увереност 0.33–0.67 (стойност на 1–2 страници/варианта) → хибриден режим.
- **Имена на кирилица/гръцки** се четат с ниска увереност → винаги проверка от човек.
- **Обработка на многостраничен PDF:** режем до първите 3 страници; изборът на най-подходяща страница
  (§3, стъпка b) не е реализиран.
- **Време:** снимка 35–50 с в Ниво 1 (бюджет 60 с); пълният пакет тестове ~8 мин.

### Остава — решения и интеграция (не са код на doc-service)

- **Поръчки, ЧМР, T1/T2, транспортни заявки** — за order-service; няма extraction contract и fleet-service
  не може да ги приеме. Корпусът (тестови образци) вече е събран.
- **Заседнали `queued` заявки:** ако doc-service е спрян над 24 ч, старите съобщения се пропускат
  (`MAX_MESSAGE_AGE_SECONDS`), а редът в `document_extractions` остава `queued` завинаги. fleet-service
  трябва да изтича стари `queued` към `failed` (или да позволява повторна заявка).
- **Фронтенд:** не съществува — качване, „изчакване“ (подписка на `fleet_extractions`), показване на
  предложението с флагове по увереност и ръчно потвърждение.
- **Портрет на водача:** `portrait.py` работи (тестван на 4 образеца), но не е свързан — отпаднал по
  решение; при връщане му трябва запис в S3 (нов префикс/права), ключ в събитието и основание за
  биометрични данни.
- **Лимити и таксуване:** `doc.ai_usage.recorded` няма консуматор; цените ($0.004/0.014/0.068) са оценка
  по брой токени, не от фактура.
- **Лични данни към Anthropic:** реалните документи съдържат данни на хора; преди продукционен трафик —
  проверка на договора за обработка на данни.
- **API ключът беше поставен в чата.** Въпреки че стои само в `.env` (извън git), препоръчвам да се смени.
- **Мащабиране:** една инстанция, `DOC_WORKERS=2`, RAM 1 GB; пренареждане на групата с няколко инстанции
  не е тествано на живо (само логиката в `dispatcher.py`/`kafka_client.py`); работници/памет се настройват
  заедно.

### Остава — поддръжка

- Корпусът `tester/fixtures/documents/` (~135 MB, смесени лицензи) е само локален и не е пресъздаваем
  автоматично — няма списък на източниците. Ако се иска споделяне: манифест с URL + Git LFS.
- §2 (ENV) по-горе е остарял по имената на S3 променливите (`DOC_S3_ACCESS_KEY`/`DOC_S3_SECRET_KEY`) и
  не описва `DOC_WORKERS`, `DOC_PER_COMPANY_MAX`, `DOC_MAX_BUFFER`, `ANTHROPIC_MODEL_ESCALATION`
  (виж `.env.example`, което е точното).
- Стойностите в `tools/eval_set.json` са четени на око от образците; при нови образци се допълват.

---

## БЪДЕЩИ ИДЕИ (извън обхвата на тази версия)

- Идемпотентен кеш по `extraction_id` (§8), ако двойното AI плащане при Kafka redelivery стане проблем.
- sha256/preview/thumbnail/PDF split пайплайн (етап 8) — отделна спецификация.
- Лимит/квота по план за AI извиквания — на billing-service; doc-service само отчита.
- Prompt caching е вързан, но при компактния речник (~4600 токена) е под прага за Haiku; при Sonnet/Opus
  и по-дълъг речник (повече типове) може да започне да се плаща 1.25× за кеш запис при рядък трафик — да
  се мери.
